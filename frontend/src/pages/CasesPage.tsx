...
const emptyForm = { patient_id: "", diagnosis: "", priority: "medium" as string, notes: "" };
...
export default function CasesPage() {
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      if (editingItem) {
        await casesApi.update(editingItem.id, {
          diagnosis: form.diagnosis, priority: form.priority, notes: form.notes || undefined,
        });
        setEditingItem(null);
      } else {
      ...
      setFormError(msg || "Failed to save case");
  ...
  const modalTitle = editingItem ? `Edit Case: ${editingItem.diagnosis}` : "New Case";
  ...
  const statusColor = (status: string) => {
    switch (status) {
      ...
      case "in_treatment": return "bg-green-100 text-green-700";
          ...
          {cases.map((c) => (
            <div key={c.id}
              ...
              <div className="flex items-center justify-between">
                <Link to={`/cases/${c.id}`} className="flex-1">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">Case ID: {c.id.slice(0, 8)}...</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${statusColor(c.status)}`}>
                        {c.status.replace("_", " ")}
                      </span>
                ...
                {isAdmin && (
                  <div className="ml-3 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEdit(c)}
                      className="px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded whitespace-nowrap">Edit</button>
                    <button onClick={() => setConfirmArchive(c.id)}
                      className="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded whitespace-nowrap">Delete</button>
                  </div>
                )}
              </div>
            </div>
          ))}
          {cases.length === 0 && <p className="text-center text-gray-500 py-8">No cases found.</p>}
        </div>
      )}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          {!editingItem && (
            <div>
              ...
              <select required value={form.patient_id}
                ...
                {patients.map((p) => (
                  <option key={p.id} value={p.id}>{p.full_name} (Age: {p.age})</option>
                ))}
              </select>
              {patients.length === 0 && <p className="text-xs text-gray-400 mt-1">No patients found — add a patient first.</p>}
            </div>
          )}
          <div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? "Saving..." : editingItem ? "Save Changes" : "Create Case"}
            </button>
          </div>
        </form>
      </Modal>
      ...
      {confirmArchive && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-sm text-gray-600 mb-4">This case and all its data will be permanently removed.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setConfirmArchive(null)} className="px-4 py-2 text-sm text-gray-600">Cancel</button>
              <button onClick={() => handleDelete(confirmArchive)}
